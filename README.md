...
- **初始版本 (ESP32CAM_Car1.020)**: 
基础功能实现，包括摄像头初始化、WiFi连接、HTTP服务器搭建、电机控制和网页界面。
  
- **稳定性优化 (ESP32CAM_Car1.03)**: 
  - 降低摄像头初始分辨率至QVGA，减少内存使用。
  - 在loop()中添加delay(10)以防止CPU占用过高。
  - 分离HTTP服务器端口（控制80，视频81）以避免阻塞。

- **流畅度优化 (ESP32CAM_Car HTTP)**: 
...
- **UI优化和控制改进 (ESP32CAM_Car WebSocket2)**: 
  - 虚拟摇杆映射顺时针旋转90度，修正前进/转向方向（向右前进，向下右转等），提升操作直观性。
  - UI界面优化：摄像头画面放大至flex比例2:1:1，摇杆和按钮填满左侧/右侧空间，保持按钮中点位置不变。
  - 添加设备方向检测，竖屏时提示用户横屏使用应用。
  - 按钮高度增加至50px，宽度100%，提升触摸友好性。
  - 摄像头流添加object-fit: contain，确保画面比例不变形。
